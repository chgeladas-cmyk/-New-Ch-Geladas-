...
const Store = (() => {
  ...
  // Cache do histórico COMPLETO de clientes fiado, buscado direto da nova
  // coleção 'fiado_clientes' (documento por cliente). null até alguma tela
  // chamar Store.carregarFiadoCompleto() — até lá, getFiado()/mutateFiado()
  // continuam operando no array local de sempre (_read('fiado')), sem
  // regressão. Depois de carregado, vira a fonte única de leitura E
  // escrita pra 'fiado' (mutateFiado passa a mutar este array).
  let _fiadoFullCache = null;

  // Mesmo mecanismo, generalizado pra 'saidas' e 'ponto' — ambos são
  // arrays achatados de registros (não aninhados como fiado), migrados
  // pro mesmo modelo de 1 documento por registro.
  let _saidasFullCache = null;
  let _pontoFullCache  = null;
  const _COLS_DIFF_SYNC = new Set(['fiado', 'saidas', 'ponto']);
  function _fullCacheDoCol(col) {
    if (col === 'fiado')  return _fiadoFullCache;
    if (col === 'saidas') return _saidasFullCache;
    if (col === 'ponto')  return _pontoFullCache;
    return null;
  }
  function _setFullCacheDoCol(col, val) {
    if (col === 'fiado')  _fiadoFullCache  = val;
    if (col === 'saidas') _saidasFullCache = val;
    if (col === 'ponto')  _pontoFullCache  = val;
  }
  ...
  // Depois que Store.carregarXCompleto() já rodou pra essa coleção, o
  // cache completo (vindo da coleção nova, documento por registro) passa
  // a ser a fonte da verdade — leitura e escrita mutam o MESMO array.
  const usaFullCache = _COLS_DIFF_SYNC.has(col);
  const cacheAtual = usaFullCache ? _fullCacheDoCol(col) : null;
  const data = cacheAtual || _read(col);

  // Snapshot ANTES da mutação, pra saber exatamente quais registros
  // mudaram (novos, editados ou excluídos) e sincronizar só esses como
  // documento individual — evita o limite de 1 MiB do documento único e
  // o custo de reenviar tudo a cada mutação de um registro só.
  let _antes = null;
  if (usaFullCache) {
    _antes = new Map(data.map(item => [item.id, JSON.stringify(item)]));
  }
  ...
  _write(col, final);
  if (usaFullCache && cacheAtual) _setFullCacheDoCol(col, final); // mantém os dois em sincronia

  if (usaFullCache) {
    const _salvarFnNome  = { fiado: 'salvarClienteFiado',  saidas: 'salvarRegistroSaida',  ponto: 'salvarRegistroPonto'  }[col];
    const _deletarFnNome = { fiado: 'deletarClienteFiado', saidas: 'deletarRegistroSaida', ponto: 'deletarRegistroPonto' }[col];
    const _salvarFn  = window.CH?.FirebaseService?.[_salvarFnNome];
    const _deletarFn = window.CH?.FirebaseService?.[_deletarFnNome];

    if (_salvarFn) {
      const _idsDepois = new Set(data.map(item => item?.id).filter(Boolean));

      data.forEach(item => {
        if (!item?.id) return;
        const depois = JSON.stringify(item);
        if (_antes.get(item.id) !== depois) {
          _salvarFn(item).catch(e => console.warn(`[${col}] Falha ao gravar documento individual:`, item.id, e.message));
        }
      });

      // Registros que existiam antes mas não existem mais (excluídos)
      // precisam ter o documento individual apagado também.
      _antes.forEach((_, idAntigo) => {
        if (!_idsDepois.has(idAntigo) && _deletarFn) {
          _deletarFn(idAntigo).catch(e => console.warn(`[${col}] Falha ao apagar documento individual:`, idAntigo, e.message));
        }
      });
    }
  }
  ...
  // 'fiado', 'financeiro', 'saidas' e 'ponto' já migraram para documento
  // individual. Sem este bloqueio, este trecho reenviaria o array inteiro
  // pro documento único antigo (ch_dados/{col}) a cada mutação —
  // RESSUSCITANDO o documento que a migração apagou, com os mesmos
  // problemas de antes (corte de quantidade, limite de 1 MiB). A
  // sincronização de verdade dessas coleções já acontece por documento
  // individual (ver bloco acima / financeiroService.js), então o caminho
  // antigo abaixo é pulado pra elas.
  const _jaMigradoParaDocumentos = new Set(['fiado', 'financeiro', 'saidas', 'ponto']);
  if (_jaMigradoParaDocumentos.has(col)) return;
  ...
  getFiado()         { return _fiadoFullCache || _read('fiado'); },
  getPonto()         { return _pontoFullCache || _read('ponto'); },
  ...
  getSaidas()        { return _saidasFullCache || _read('saidas'); },
  ...
  // Busca o histórico COMPLETO de clientes fiado direto da nova coleção
  // 'fiado_clientes' (documento por cliente, sem o teto de 1 MiB do
  // documento único). Chamado explicitamente pelas telas que precisam de
  // fiado correto (fiado.html, vendas.html) — não roda sozinho em todo
  // carregamento pra não gastar leitura à toa em telas que não usam fiado.
  // Depois de chamado, getFiado()/mutateFiado() passam a usar este cache.
  async carregarFiadoCompleto() {
    if (!window.CH?.FirebaseService?.lerClientesFiado) return null; // fallback seguro
    try {
      _fiadoFullCache = await window.CH.FirebaseService.lerClientesFiado();
      _notify('fiado');
      return _fiadoFullCache;
    } catch (e) {
      console.warn('[Fiado] Falha ao buscar histórico completo, usando cache local (pode estar incompleto):', e.message);
      return null;
    }
  },

  // Mesmo mecanismo pra 'saidas' e 'ponto' — documento por registro em
  // 'saidas_registros'/'ponto_registros', sem o corte de 5.000/1.000 do
  // documento único antigo.
  async carregarSaidasCompleto() {
    if (!window.CH?.FirebaseService?.lerRegistrosSaidas) return null;
    try {
      _saidasFullCache = await window.CH.FirebaseService.lerRegistrosSaidas();
      _notify('saidas');
      return _saidasFullCache;
    } catch (e) {
      console.warn('[Saidas] Falha ao buscar histórico completo, usando cache local (pode estar incompleto):', e.message);
      return null;
    }
  },

  async carregarPontoCompleto() {
    if (!window.CH?.FirebaseService?.lerRegistrosPonto) return null;
    try {
      _pontoFullCache = await window.CH.FirebaseService.lerRegistrosPonto();
      _notify('ponto');
      return _pontoFullCache;
    } catch (e) {
      console.warn('[Ponto] Falha ao buscar histórico completo, usando cache local (pode estar incompleto):', e.message);
      return null;
    }
  },
...
const FirebaseService = (() => {
  ...
  // ── Leitura completa e paginada de 'vendas' + 'historico_vendas' ───────
  // BUG CORRIGIDO: o cache local (Store.getVendas(), alimentado pelo
  // onSnapshot com limit(1000) logo acima) só guarda as 1000 vendas mais
  // recentes. Com volumetria alta (15k-50k+ vendas), relatórios financeiros
  // que somam direto de Store.getVendas() — inclusive o período "Geral" —
  // ficavam restritos a ~2 meses de histórico, subestimando faturamento
  // real de forma grave (ex.: R$13k exibido vs R$50k+ real).
  //
  // Esta função busca DIRETO do Firestore, paginada via cursor (startAfter),
  // sem limit artificial, e NÃO grava o resultado no localStorage (evita
  // estourar a quota de localStorage do navegador com 15k+ documentos).
  // Deve ser usada por telas de relatório/financeiro que precisam do
  // histórico completo e correto, não pela operação do PDV em si.
  //
  // Lê as DUAS coleções — 'vendas' (ativa/operacional) e 'historico_vendas'
  // (dados arquivados manualmente pela aba de arquivamento, ver
  // arquivarVendasAntigas) — e mescla o resultado, para que os relatórios
  // continuem 100% corretos mesmo depois de um arquivamento.
  const _COLECAO_VENDAS    = 'vendas';
  const _COLECAO_HISTORICO = 'historico_vendas';

  async function _lerColecaoPaginada(nomeColecao, dataDeISO, dataAteISO) {
    const PAGINA = 500;
    const vendas = [];
    let cursor = null;

    const filtros = [];
    if (dataDeISO)  filtros.push(_fb.where('criadoEm', '>=', dataDeISO));
    if (dataAteISO) filtros.push(_fb.where('criadoEm', '<=', dataAteISO + 'T23:59:59.999'));

    while (true) {
      const partes = [
        _fb.collection(_db, nomeColecao),
        ...filtros,
        _fb.orderBy('criadoEm', 'asc'),
        _fb.limit(PAGINA),
      ];
      if (cursor) partes.push(_fb.startAfter(cursor));

      let snap;
      try {
        snap = await _fb.getDocs(_fb.query(...partes));
      } catch (e) {
        // Coleção pode não existir ainda (nenhum arquivamento feito) ou
        // faltar permissão nas regras do Firestore — não derruba o
        // relatório inteiro por causa disso.
        console.warn(`[Firebase] Falha ao ler ${nomeColecao}:`, e.code || e.message);
        break;
      }
      if (snap.empty) break;
      ...
      snap.docs.forEach(d => {
        const v = d.data();
        if (!v._deleted) vendas.push(v);
      });

      if (snap.docs.length < PAGINA) break; // última página
  ...
  async function lerVendasPeriodo(dataDeISO, dataAteISO) {
    _requireReady();
    const [historicas, ativas] = await Promise.all([
      _lerColecaoPaginada(_COLECAO_HISTORICO, dataDeISO, dataAteISO),
      _lerColecaoPaginada(_COLECAO_VENDAS, dataDeISO, dataAteISO),
    ]);
    return [...historicas, ...ativas];
  }

  // Atalho para "todo o período" (equivalente ao filtro "Geral" da UI).
  async function lerVendasCompleto() {
    return lerVendasPeriodo(null, null);
  }

  // ── Contagem barata (via count() agregado no servidor) ─────────────────
  // Usada pela aba de arquivamento para decidir se já vale a pena arquivar.
  async function contarVendas() {
    _requireReady();
    const snap = await _fb.getCountFromServer(_fb.collection(_db, _COLECAO_VENDAS));
  ...
  async function contarHistorico() {
    _requireReady();
    try {
      const snap = await _fb.getCountFromServer(_fb.collection(_db, _COLECAO_HISTORICO));
      return snap.data().count;
    } catch (e) {
      return 0; // coleção ainda não existe
    }
  }

  // ── Arquivamento manual: vendas -> historico_vendas ─────────────────────
  // Mantém as `manterRecentes` vendas mais novas na coleção operacional
  // 'vendas' (rápida, usada pelo PDV e pelo cache em tempo real) e move o
  // restante para 'historico_vendas' (só lida sob demanda pelos relatórios,
  // nunca pelo onSnapshot do PDV). NADA é perdido — o histórico completo
  // continua acessível via lerVendasPeriodo/lerVendasCompleto, que já lêem
  // as duas coleções.
  //
  // Segurança: só deleta da coleção de origem depois de confirmar (commit)
  // a escrita no destino. Se a escrita falhar, o lote inteiro é abortado
  // sem apagar nada — nunca existe um estado onde um documento é perdido.
  //
  // onProgresso(info) é chamado a cada etapa para a UI mostrar feedback:
  //   { fase:'inicio', total, qtdParaArquivar }
  //   { fase:'progresso', arquivados, qtdParaArquivar }
  //   { fase:'erro_escrita'|'erro_delete', erro, idsAfetados? }
  //   { fase:'fim', arquivados, totalDepois }
  async function arquivarVendasAntigas({ manterRecentes = 2000, tamanhoLote = 400, onProgresso } = {}) {
    _requireReady();

    const total = await contarVendas();
    if (total <= manterRecentes) {
      const info = { arquivados: 0, totalAntes: total, totalDepois: total,
        mensagem: `Nada a arquivar — a coleção tem ${total} vendas, dentro do limite de ${manterRecentes}.` };
      onProgresso?.({ fase: 'fim', ...info });
      return info;
    }

    const qtdParaArquivar = total - manterRecentes;
    onProgresso?.({ fase: 'inicio', total, qtdParaArquivar });

    let arquivados = 0;
    let cursor = null;

    while (arquivados < qtdParaArquivar) {
      const restante = qtdParaArquivar - arquivados;
      const tamanhoPagina = Math.min(tamanhoLote, restante);

      const partes = [
        _fb.collection(_db, _COLECAO_VENDAS),
        _fb.orderBy('criadoEm', 'asc'), // arquiva sempre as mais antigas primeiro
        _fb.limit(tamanhoPagina),
      ];
      if (cursor) partes.push(_fb.startAfter(cursor));

      const snap = await _fb.getDocs(_fb.query(...partes));
      if (snap.empty) break; // segurança — não deveria acontecer

      // Etapa 1: grava no histórico (mesmo ID, para rastreabilidade)
      const batchEscrita = _fb.writeBatch(_db);
      snap.docs.forEach(d => {
        const refHist = _fb.doc(_db, _COLECAO_HISTORICO, d.id);
        batchEscrita.set(refHist, { ...d.data(), _arquivadoEm: new Date().toISOString() });
      });

      try {
        await batchEscrita.commit();
      } catch (e) {
        onProgresso?.({ fase: 'erro_escrita', erro: e.message });
        break; // aborta — nada deste lote é deletado da origem
      }

      // Etapa 2: só deleta da origem após confirmar a escrita no destino
      const batchDelete = _fb.writeBatch(_db);
      snap.docs.forEach(d => batchDelete.delete(d.ref));

      try {
        await batchDelete.commit();
        arquivados += snap.docs.length;
        cursor = snap.docs[snap.docs.length - 1];
        onProgresso?.({ fase: 'progresso', arquivados, qtdParaArquivar });
      } catch (e) {
        onProgresso?.({ fase: 'erro_delete', erro: e.message, idsAfetados: snap.docs.map(d => d.id) });
        break; // para — documentos deste lote ficam duplicados (histórico + origem), revisar manualmente
      }
    }

    const totalDepois = await contarVendas();
    const resultado = { arquivados, totalAntes: total, totalDepois };
    onProgresso?.({ fase: 'fim', ...resultado });
    return resultado;
  }

  // ── LANÇAMENTOS FINANCEIROS: documento individual por lançamento ──────
  // BUG CORRIGIDO: a coleção 'financeiro' vivia inteira num único documento
  // (ch_dados/financeiro), com um array que o core.js cortava em 5.000 itens
  // (os mais antigos eram descartados — inclusive no Firestore, não só
  // localmente). Somado a isso, financeiroService.getLancamentos() aplicava
  // um segundo corte de 500 por chamada. Resultado: relatórios "Geral"
  // pareciam travados num valor, porque a soma nunca refletia o histórico
  // real — só uma fatia rotativa dele.
  //
  // Novo modelo: coleção 'financeiro_lancamentos', 1 documento por
  // lançamento (mesmo ID que já existia), sem corte de quantidade.
  const _COLECAO_LANCAMENTOS = 'financeiro_lancamentos';

  async function salvarLancamentoFinanceiro(lancamento) {
    _requireReady();
    if (!lancamento?.id) throw new Error('Lançamento precisa de um id.');
    await _fb.setDoc(_fb.doc(_db, _COLECAO_LANCAMENTOS, lancamento.id), lancamento);
    return true;
  }

  async function lerLancamentosFinanceiro({ dataDe, dataAte } = {}) {
    _requireReady();

    // BUG CRÍTICO EVITADO AQUI: antes da migração manual rodar, a coleção
    // financeiro_lancamentos está vazia. Sem mesclar com o array antigo
    // (ch_dados/financeiro), esta função retornaria [] assim que o app for
    // atualizado — e como o cache é truthy mesmo vazio, o financeiro.html
    // mostraria histórico zerado ANTES de você rodar a migração. Por isso
    // lê as duas fontes sempre, e mescla por id (documento novo tem
    // prioridade em caso de sobreposição).
    let antigos = [];
    try {
      const snapAntigo = await _fb.getDoc(_fb.doc(_db, 'ch_dados', 'financeiro'));
      if (snapAntigo.exists()) antigos = snapAntigo.data()?.dados || [];
    } catch (e) {
      console.warn('[Firebase] Falha ao ler ch_dados/financeiro (fallback):', e.code || e.message);
    }
    ...
    while (true) {
      const partes = [
        _fb.collection(_db, _COLECAO_LANCAMENTOS),
        _fb.orderBy('dataCurta', 'asc'),
        _fb.limit(PAGINA),
      ];
      if (cursor) partes.push(_fb.startAfter(cursor));
      ...
      try {
        ...
        console.warn(`[Firebase] Falha ao ler ${_COLECAO_LANCAMENTOS}:`, e.code || e.message);
        break;
      }
      if (snap.empty) break;

      snap.docs.forEach(d => novos.push(d.data()));

      if (snap.docs.length < PAGINA) break;
      cursor = snap.docs[snap.docs.length - 1];
    }

    // Mescla por id — se um lançamento existir nas duas fontes (não deveria,
    // mas por segurança), o documento novo (mais recente) vence.
    const porId = new Map();
    antigos.forEach(l => { if (l?.id) porId.set(l.id, l); });
    novos.forEach(l   => { if (l?.id) porId.set(l.id, l); });

    let lancamentos = Array.from(porId.values());
    if (dataDe)  lancamentos = lancamentos.filter(l => l.dataCurta >= dataDe);
    if (dataAte) lancamentos = lancamentos.filter(l => l.dataCurta <= dataAte);

    return lancamentos;
  }

  // ── Migração única: ch_dados/financeiro (array) -> financeiro_lancamentos (docs) ──
  // Mesma segurança do arquivamento de vendas: só apaga o array antigo depois
  // de confirmar que TODOS os lançamentos foram gravados como documentos.
  async function migrarFinanceiroParaDocumentos({ onProgresso, tamanhoLote = 400 } = {}) {
    _requireReady();

    let snapAntigo;
    try {
      snapAntigo = await _fb.getDoc(_fb.doc(_db, 'ch_dados', 'financeiro'));
    } catch (e) {
      throw new Error('Falha ao ler o documento antigo ch_dados/financeiro: ' + e.message);
    }

    if (!snapAntigo.exists()) {
      return { migrados: 0, mensagem: 'ch_dados/financeiro não existe — nada a migrar.' };
    }

    const arrayAntigo = snapAntigo.data()?.dados;
    if (!Array.isArray(arrayAntigo) || arrayAntigo.length === 0) {
      return { migrados: 0, mensagem: 'Array de financeiro está vazio — nada a migrar.' };
    }

    const total = arrayAntigo.length;
    onProgresso?.({ fase: 'inicio', total });

    let migrados = 0;
    for (let i = 0; i < arrayAntigo.length; i += tamanhoLote) {
      const fatia = arrayAntigo.slice(i, i + tamanhoLote);
      const batch = _fb.writeBatch(_db);

      fatia.forEach(lancamento => {
        if (!lancamento?.id) return; // segurança — pula registros sem id
        const ref = _fb.doc(_db, _COLECAO_LANCAMENTOS, lancamento.id);
        batch.set(ref, lancamento);
      });

      try {
        await batch.commit();
        migrados += fatia.length;
        onProgresso?.({ fase: 'progresso', migrados, total });
      } catch (e) {
        onProgresso?.({ fase: 'erro_escrita', erro: e.message, loteIndex: i });
        // Aborta sem apagar nada do documento antigo — nenhum dado é perdido.
        return { migrados, total, sucesso: false, erro: e.message };
      }
    }

    // Só apaga o documento antigo depois de confirmar que TODOS foram gravados.
    if (migrados === total) {
      try {
        await _fb.deleteDoc(_fb.doc(_db, 'ch_dados', 'financeiro'));
      } catch (e) {
        // Não é crítico — os dados novos já estão seguros; o doc antigo
        // só fica como remanescente pra apagar manualmente depois.
        onProgresso?.({ fase: 'aviso_delete_antigo', erro: e.message });
      }
    }

    const resultado = { migrados, total, sucesso: migrados === total };
    onProgresso?.({ fase: 'fim', ...resultado });
    return resultado;
  }

  // ── FIADO: documento individual por CLIENTE ─────────────────────────
  // fiado NÃO é uma lista de vendas fiado — é uma lista de clientes, cada
  // um com seu próprio histórico embutido (movimentacoes[], cancelamentos[]).
  // Por isso a migração é 1 documento por CLIENTE (preserva a forma atual
  // dos dados), não 1 documento por transação. Resolve o limite de 1 MiB
  // do documento único (ch_dados/fiado, que hoje guarda TODOS os clientes)
  // e o custo de reenviar todo mundo a cada mutação de um cliente só.
  const _COLECAO_FIADO_CLIENTES = 'fiado_clientes';

  async function salvarClienteFiado(cliente) {
    _requireReady();
    if (!cliente?.id) throw new Error('Cliente de fiado precisa de um id.');
    await _fb.setDoc(_fb.doc(_db, _COLECAO_FIADO_CLIENTES, cliente.id), cliente);
    return true;
  }

  async function deletarClienteFiado(clienteId) {
    _requireReady();
    await _fb.deleteDoc(_fb.doc(_db, _COLECAO_FIADO_CLIENTES, clienteId));
    return true;
  }

  async function lerClientesFiado() {
    _requireReady();

    // Mesmo bug crítico evitado aqui que em lerLancamentosFinanceiro: sem
    // mesclar com o array antigo (ch_dados/fiado), a lista de clientes
    // ficaria vazia assim que este código for publicado — quebrando a
    // venda fiado (nenhum cliente pra selecionar) ANTES de rodar a migração.
    let antigos = [];
    try {
      const snapAntigo = await _fb.getDoc(_fb.doc(_db, 'ch_dados', 'fiado'));
      if (snapAntigo.exists()) antigos = snapAntigo.data()?.dados || [];
    } catch (e) {
      console.warn('[Firebase] Falha ao ler ch_dados/fiado (fallback):', e.code || e.message);
    }
    ...
    while (true) {
      const partes = [
        _fb.collection(_db, _COLECAO_FIADO_CLIENTES),
        _fb.orderBy('nome', 'asc'),
        _fb.limit(PAGINA),
      ];
      if (cursor) partes.push(_fb.startAfter(cursor));
      ...
      try {
        ...
        console.warn(`[Firebase] Falha ao ler ${_COLECAO_FIADO_CLIENTES}:`, e.code || e.message);
        break;
      }
      if (snap.empty) break;
    ...
    // Mescla por id — documento novo vence em caso de sobreposição.
    const porId = new Map();
    antigos.forEach(c => { if (c?.id) porId.set(c.id, c); });
    novos.forEach(c   => { if (c?.id) porId.set(c.id, c); });
    const clientes = Array.from(porId.values());

    return clientes;
  }

  // ── Migração única: ch_dados/fiado (array de clientes) -> fiado_clientes (docs) ──
  // Mesma segurança das outras migrações: só apaga o array antigo depois de
  // confirmar que TODOS os clientes foram gravados como documentos.
  async function migrarFiadoParaDocumentos({ onProgresso, tamanhoLote = 400 } = {}) {
    _requireReady();

    let snapAntigo;
    try {
      snapAntigo = await _fb.getDoc(_fb.doc(_db, 'ch_dados', 'fiado'));
    } catch (e) {
      throw new Error('Falha ao ler o documento antigo ch_dados/fiado: ' + e.message);
    }

    if (!snapAntigo.exists()) {
      return { migrados: 0, mensagem: 'ch_dados/fiado não existe — nada a migrar.' };
    }

    const arrayAntigo = snapAntigo.data()?.dados;
    if (!Array.isArray(arrayAntigo) || arrayAntigo.length === 0) {
      return { migrados: 0, mensagem: 'Array de fiado está vazio — nada a migrar.' };
    }
    ...
    for (let i = 0; i < arrayAntigo.length; i += tamanhoLote) {
      ...
      fatia.forEach(cliente => {
        if (!cliente?.id) return; // segurança — pula registros sem id
        const ref = _fb.doc(_db, _COLECAO_FIADO_CLIENTES, cliente.id);
    ...
    if (migrados === total) {
      try {
        await _fb.deleteDoc(_fb.doc(_db, 'ch_dados', 'fiado'));
      } catch (e) {
        onProgresso?.({ fase: 'aviso_delete_antigo', erro: e.message });
      }
    }
  ...
  // ── SAÍDAS e PONTO: documento individual por registro ──────────────
  // Ambas são arrays achatados (não aninhados como fiado) — mesmo padrão
  // de 'financeiro': 1 documento por registro. Função genérica reaproveitada
  // pelas duas, pra não duplicar a mesma lógica de leitura/migração 2x.
  function _criarFuncoesColecaoIndividual(nomeColecaoNova, nomeColAntiga, campoOrdenacao) {
    async function salvar(registro) {
      _requireReady();
      if (!registro?.id) throw new Error(`Registro de ${nomeColecaoNova} precisa de um id.`);
      await _fb.setDoc(_fb.doc(_db, nomeColecaoNova, registro.id), registro);
      return true;
    }

    async function deletar(registroId) {
      _requireReady();
      await _fb.deleteDoc(_fb.doc(_db, nomeColecaoNova, registroId));
      return true;
    }

    // Lê a coleção nova + mescla com o array antigo (ch_dados/{col}) —
    // essencial ANTES de rodar a migração manual, senão a tela ficaria
    // vazia assim que este código for publicado (mesmo bug já corrigido
    // em financeiro/fiado).
    async function ler() {
      _requireReady();
      ...
      let antigos = [];
      try {
        const snapAntigo = await _fb.getDoc(_fb.doc(_db, 'ch_dados', nomeColAntiga));
        if (snapAntigo.exists()) antigos = snapAntigo.data()?.dados || [];
      } catch (e) {
        console.warn(`[Firebase] Falha ao ler ch_dados/${nomeColAntiga} (fallback):`, e.code || e.message);
      }
      ...
      while (true) {
        const partes = [
          _fb.collection(_db, nomeColecaoNova),
          _fb.orderBy(campoOrdenacao, 'asc'),
          _fb.limit(PAGINA),
        ];
        if (cursor) partes.push(_fb.startAfter(cursor));
        ...
        try {
          ...
          console.warn(`[Firebase] Falha ao ler ${nomeColecaoNova}:`, e.code || e.message);
          break;
        }
        if (snap.empty) break;
      ...
      const porId = new Map();
      antigos.forEach(item => { if (item?.id) porId.set(item.id, item); });
      novos.forEach(item   => { if (item?.id) porId.set(item.id, item); });
      return Array.from(porId.values());
    }

    // Migração única: array antigo -> documento por registro. Só apaga o
    // array antigo depois de confirmar que TODOS os registros foram
    // gravados como documentos.
    async function migrar({ onProgresso, tamanhoLote = 400 } = {}) {
      _requireReady();

      let snapAntigo;
      try {
        snapAntigo = await _fb.getDoc(_fb.doc(_db, 'ch_dados', nomeColAntiga));
      } catch (e) {
        throw new Error(`Falha ao ler o documento antigo ch_dados/${nomeColAntiga}: ` + e.message);
      }

      if (!snapAntigo.exists()) {
        return { migrados: 0, mensagem: `ch_dados/${nomeColAntiga} não existe — nada a migrar.` };
      }

      const arrayAntigo = snapAntigo.data()?.dados;
      if (!Array.isArray(arrayAntigo) || arrayAntigo.length === 0) {
        return { migrados: 0, mensagem: `Array de ${nomeColAntiga} está vazio — nada a migrar.` };
      }
      ...
      for (let i = 0; i < arrayAntigo.length; i += tamanhoLote) {
        ...
        fatia.forEach(item => {
          if (!item?.id) return;
          const ref = _fb.doc(_db, nomeColecaoNova, item.id);
          batch.set(ref, item);
        });
      ...
      if (migrados === total) {
        try {
          await _fb.deleteDoc(_fb.doc(_db, 'ch_dados', nomeColAntiga));
        } catch (e) {
          onProgresso?.({ fase: 'aviso_delete_antigo', erro: e.message });
        }
      }
    ...
    return { salvar, deletar, ler, migrar };
  }

  const _saidasDocs = _criarFuncoesColecaoIndividual('saidas_registros', 'saidas', 'criadoEm');
  const _pontoDocs  = _criarFuncoesColecaoIndividual('ponto_registros',  'ponto',  'criadoEm');

  async function salvarRegistroSaida(saida)     { return _saidasDocs.salvar(saida); }
  async function deletarRegistroSaida(id)       { return _saidasDocs.deletar(id); }
  async function lerRegistrosSaidas()           { return _saidasDocs.ler(); }
  async function migrarSaidasParaDocumentos(o)  { return _saidasDocs.migrar(o); }

  async function salvarRegistroPonto(reg)       { return _pontoDocs.salvar(reg); }
  async function deletarRegistroPonto(id)       { return _pontoDocs.deletar(id); }
  async function lerRegistrosPonto()            { return _pontoDocs.ler(); }
  async function migrarPontoParaDocumentos(o)   { return _pontoDocs.migrar(o); }

  return {
  init, salvar, ler, deletar, atualizar,
  lerVendasPeriodo, lerVendasCompleto, contarVendas, contarHistorico, arquivarVendasAntigas,
  salvarLancamentoFinanceiro, lerLancamentosFinanceiro, migrarFinanceiroParaDocumentos,
  salvarClienteFiado, deletarClienteFiado, lerClientesFiado, migrarFiadoParaDocumentos,
  salvarRegistroSaida, deletarRegistroSaida, lerRegistrosSaidas, migrarSaidasParaDocumentos,
  salvarRegistroPonto, deletarRegistroPonto, lerRegistrosPonto, migrarPontoParaDocumentos,
  isReady: () => _ready,
